...
def main():
    ...
    args.store_name = '_'.join(
        [args.dataset, args.arch, args.loss_type, args.train_rule, args.data_aug, str(args.imb_factor),
         str(args.rand_number), args.exp_str])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint(args, {
            ...
            'best_acc1': best_acc1,}, is_best, epoch + 1)
    end_time = time.time()
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, tf_writer, weighted_train_loader=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.data_aug == 'MosaicMix' and args.start_data_aug < epoch < (args.epochs - args.end_data_aug):
            idx1 = torch.randperm(input.size(0))
            idx2 = torch.randperm(input.size(0))
            idx3 = torch.randperm(input.size(0))
            image_a, image_b, image_c, image_d = input, input[idx1], input[idx2], input[idx3]
            label_a, label_b, label_c, label_d = target, target[idx1], target[idx2], target[idx3]
            ncls_list = torch.FloatTensor(args.cls_num_list)
            n_a = ncls_list[label_a]
            if len(input.size()) == 4:
                w = input.size()[2]
                h = input.size()[3]
            ...
            # Set the range of the delimiter position for concatenation.
            min_offset_x = 0.1
            min_offset_y = 0.1
            ...
            for p, pic in enumerate(image_a):
                if n_a[p] >= 58:
                    if np.random.randint(3) == 1:  # ImageNet   41 1/4
                    # There's a 33% chance to combine four images.
                    # If the random number is 1, mix the four images;
                        image_a[p][:, :cuty, :cutx] = image_b[p][:, :cuty, :cutx]
                        image_a[p][:, cuty:, :cutx] = image_c[p][:, cuty:, :cutx]
                        image_a[p][:, cuty:, cutx:] = image_d[p][:, cuty:, cutx:]
                    else:
                    # otherwise, mix two of the images.
                        if index == 0:
                            # new_frame[:, :, 0:nh, 0:nw] = frame  # Upper left corner.
                            image_a[p, :, :cuty, :cutx] = image_b[p, :, :cuty, :cutx]
                            label_c[p] = label_a[p]
                            label_d[p] = label_a[p]
                            label_a[p] = label_b[p]
                            ...
                            # new_frame[:, :, 0:nh, w - nw:w] = frame  # Upper right corner.
                            image_a[p, :, :cuty, cutx:] = image_b[p, :, :cuty, cutx:]
                            label_c[p] = label_a[p]
                            label_d[p] = label_a[p]
                        elif index == 2:
                            # new_frame[:, :, h - nh:h, 0:nw] = frame  # Lower left corner.
                            image_a[p, :, cuty:, :cutx] = image_b[p, :, cuty:, :cutx]
                            label_b[p] = label_a[p]
                            label_d[p] = label_a[p]
                        elif index == 3:
                            # new_frame[:, :, h - nh:h, w - nw:w] = frame  # Lower right corner.
                            image_a[p, :, cuty:, cutx:] = image_b[p, :, cuty:, cutx:]
                            label_b[p] = label_a[p]
                            label_c[p] = label_a[p]
                else:
            ...
            loss = (l1 / z) * criterion(output, label_a).mean() + \
                   (l2 / z) * criterion(output, label_b).mean() + \
                   (l3 / z) * criterion(output, label_c).mean() + \
                   (l4 / z) * criterion(output, label_d).mean()
        else:
            output = model(input)
